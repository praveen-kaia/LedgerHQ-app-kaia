...
typedef enum {
    ...
    PARTIAL_FEE_DELEGATED_VALUE_TRANSFER =
        0x0A,                    // Partial fee-delegated value transfer transaction type.
    VALUE_TRANSFER_MEMO = 0x10,  // Value transfer with memo transaction type.
    FEE_DELEGATED_VALUE_TRANSFER_MEMO =
        0x11,  // Fee-delegated value transfer with memo transaction type.
    PARTIAL_FEE_DELEGATED_VALUE_TRANSFER_MEMO =
        0x12,  // Partial fee-delegated value transfer with memo transaction type.
    SMART_CONTRACT_DEPLOY = 0x28,  // Smart contract deployment transaction type.
    FEE_DELEGATED_SMART_CONTRACT_DEPLOY =
        0x29,  // Fee-delegated smart contract deployment transaction type.
    PARTIAL_FEE_DELEGATED_SMART_CONTRACT_DEPLOY =
        0x2A,  // Partial fee-delegated smart contract deployment transaction type.
    SMART_CONTRACT_EXECUTION = 0x30,  // Smart contract execution transaction type.
    FEE_DELEGATED_SMART_CONTRACT_EXECUTION =
        0x31,  // Fee-delegated smart contract execution transaction type.
    PARTIAL_FEE_DELEGATED_SMART_CONTRACT_EXECUTION =
        0x32,       // Partial fee-delegated smart contract execution transaction type.
